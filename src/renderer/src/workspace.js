...
angular
    ...
    .controller('WorkspaceController', [
        ...
        '$timeout',
        'myProject',
        function($scope, $rootScope, $modal, $http, $timeout, myProject) {
            $scope.saveFileName = '';
            ...
            $scope.initWorkspace = function() {
                ...
                Entry.addEventListener(
                    'showLoadingPopup',
                    $scope.showLoadingPopup
                );
                Entry.addEventListener(
                    'errorLoadingPopup',
                    $scope.errorLoadingPopup
                );
                Entry.addEventListener(
                    'hideLoadingPopup',
                ...
                Entry.plugin.init(function(isPracticalCourseMode) {
                    ...
                    myProject.isSavedPath =
                        storage.getItem('defaultPath') || '';
                    ...
                    Entry.playground.downloadPicture = function(pictureId) {
                        const picture = Entry.playground.object.getPicture(
                            pictureId
                        );
                        let fileurl;
                    ...
                    var localStorageProject = storage.getItem(
                        'localStorageProject'
                    );
                    ...
                    Entry.addEventListener(
                        'openSpriteManager',
                        $scope.openSpriteManager
                    );
                    Entry.addEventListener(
                        'openPictureManager',
                        $scope.openPictureManager
                    );
                    Entry.addEventListener(
                        'openSoundManager',
                        $scope.openSoundManager
                    );
                    Entry.addEventListener(
                        'changeVariableName',
                        $scope.changeVariableName
                    );
                    Entry.addEventListener(
                        'deleteMessage',
                        $scope.deleteMessage
                    ...
                    Entry.addEventListener(
                        'saveCanvasImage',
                        $scope.saveCanvasData
                    );
                    Entry.addEventListener(
                        'openPictureImport',
                        $scope.openPictureImport
                    );
                    Entry.addEventListener(
                    ...
                    Entry.getMainWS().changeEvent.attach(this, () => {
                        $timeout(() => {
                            $scope.$apply(() => {
                                myProject.programmingMode = Number(
                                    Entry.getMainWS().mode
                                );
                            });
                        }, 0);
                    });
                ...
                $('#entryCategoryarduino').mouseup(function() {
                    ...
                    if (user_lang === 'ko') {
                        Lang.Blocks.ARDUINO_download_connector =
                            '하드웨어 플러그인 열기';
                        Lang.Blocks.ARDUINO_download_source =
                            '아두이노 소스코드 받기';
                        Lang.Blocks.ARDUINO_reconnect = '하드웨어에 연결하기';
                    } else {
                        Lang.Blocks.ARDUINO_download_connector =
                            'Open Hardware Plugin';
                        Lang.Blocks.ARDUINO_download_source =
                            'Get Arduino Code';
            ...
            $scope.setWorkspace = function(project, b) {
                ...
                if (!project) {
                    ...
                    project.objects[0] = getTranslatedObject(
                        project.objects[0]
                    );
                    project.scenes[0] = getTranslatedScene(project.scenes[0]);
                    var i = Math.floor(
                        Math.random() * Lang.Workspace.PROJECTDEFAULTNAME.length
                    );
                    project_name =
                        Lang.Workspace.PROJECTDEFAULTNAME[i] +
                        ' ' +
                        Lang.Workspace.project;
                ...
                $scope.project.name =
                    project_name || Lang.Workspace.new_project;
            ...
            $scope.saveWorkspace = function() {
                ...
                if (myProject.isSaved && parseDir.ext === '.ent') {
                    $scope.project.saveProject(
                        myProject.isSavedPath,
                        function() {
                            Entry.stateManager.addStamp();
                            Entry.toast.success(
                                Lang.Workspace.saved,
                                myProject.name + ' ' + Lang.Workspace.saved_msg
                            );
                            $scope.doPopupControl({
                                type: 'hide',
                            });
                    ...
                    Entry.plugin.loadProject(filePath, function(e, data) {
                        if (e) {
                            ...
                            jsonObj.objects.forEach(function(object) {
                                ...
                                sprite.pictures.forEach(function(picture) {
                                    if (picture.fileurl) {
                                        ...
                                        if (tempIndex > -1) {
                                            if (tempIndex > 0) {
                                                tempPath = picture.fileurl.substr(
                                                    tempIndex - 1
                                                );
                                            }
                                            picture.fileurl = path.posix.join(
                                                _real_temp_path_posix,
                                                tempPath
                                            );
                                        }
                                    }
                            ...
                            localStorage.setItem(
                                'isPracticalCourse',
                                jsonObj.isPracticalCourse
                            );
                            if (
                                jsonObj.objects[0] &&
                                jsonObj.objects[0].script.substr(0, 4) ===
                                    '<xml'
                            ) {
                                blockConverter.convert(jsonObj, function(
                                    result
                                ) {
                                    storage.setItem(
                                        'nativeLoadProject',
                ...
                modalInstance.result.then(function(selectedItems) {
                    if (selectedItems.target === 'newSprite') {
                        ...
                        selectedItems.data.forEach(function(items) {
                            ...
                            const spritePromise = new Promise(
                                (resolve, reject) => {
                                    try {
                                        Entry.plugin.uploadTempImageFileByObject(
                                            images,
                                            resolve
                                        );
                                    } catch (e) {
                        ...
                        selectedItems.data.forEach(function(
                            item,
                            index,
                            array
                        ) {
                            if ('sprite' in item) {
                                ...
                                if (
                                    ...
                                    (!Entry.TextCodingUtil.canUsePythonVariables(
                                        variables
                                    ) ||
                                        !Entry.TextCodingUtil.canUsePythonFunctions(
                                            functions
                                        ))
                                ) {
                                    return entrylms.alert(
                                        Lang.Menus.object_import_syntax_error
                                    );
                                }
                                ...
                                if (item.objectType === 'textBox') {
                                    ...
                                    objects.forEach(function(object) {
                                        if (
                                            Entry.container.getObject(object.id)
                                        ) {
            ...
            $scope.openSoundManager = function() {
                ...
                modalInstance.result.then(function(selectedItems) {
                    if (
                        ...
                        Entry.plugin.uploadTempSoundFileByObject(
                            sounds,
                            (data) => {
                                data.forEach((item) => {
            ...
            $scope.saveCanvasData = function(data) {
                ...
                cropImageFromCanvas(data.image).then(function(trim_image_data) {
                    ...
                    tempImg.onload = function() {
                        ...
                        var image_data_url = {};
                        //일반 이미지
                        image_data_url.org = Entry.plugin.getResizeImageFromBase64(
                            tempImg,
                            canvas,
                            TARGET_SIZE
                        );
                        //섬네일 이미지
                        image_data_url.thumb = Entry.plugin.getResizeImageFromBase64(
                            tempImg,
                            canvas,
                            THUMB_SIZE
                        );

                        Entry.plugin.saveTempImageFile(image_data_url, function(
                            picture
                        ) {
                            if (file.mode === 'new') {
                            ...
                            image.onload = function(e) {
                                Entry.container.cachePicture(
                                    picture.id +
                                        Entry.playground.object.entity.id,
                                    image
                                );

                                if (
                                    Entry.playground.painter.file.id ===
                                    picture.id
                                ) {
            ...
            $scope.hwChanged = function() {
                ...
                if (Entry.hw.connected && Entry.hw.hwModule) {
                    if (
                        EntryStatic.hwMiniSupportList.indexOf(
                            Entry.hw.hwModule.name
                        ) > -1
                    ) {
            ...
            function checkShowPracticalNoticePopup() {
                if (isPracticalCourse === true) {
                    var practicalCourseNotice = localStorage.getItem(
                        'practicalCourseNotice'
                    );
            ...
            function showPracticalNoticePopup() {
                $scope.practicalTooltip = new Entry.Tooltip(
                    [
                        {
                            content: Lang.Workspace.practical_course_tooltip,
                            target: $('.modeLabel'),
                            direction: 'down',
                        },
                    ],
                ...
                $scope.popupHelper.addPopup('workspaceModeSelect', {
                    setPopupLayout: function(popup) {
                        ...
                        var modeDefault = Entry.Dom('div', {
                            classes: [
                                'workspaceModeSelectDefault',
                                'active',
                                'workspaceModeSelectBox',
                            ],
                        ...
                        var modePracticalArts = Entry.Dom('div', {
                            classes: [
                                'workspaceModeSelectPracticalArts',
                                'workspaceModeSelectBox',
                            ],
                            parent: modeSelectArea,
                        });
            ...
            function saveAsProject(title) {
                var default_path = storage.getItem('defaultPath') || '';
                var projectName = $scope.project.name.replace(
                    /[\\\/:*?"<>|]/gi,
                    ''
                );
                ...
                Util.showSaveDialog(
                    ...
                    function(filePath) {
                        if (filePath) {
                            ...
                            try {
                                myProject.saveProject(filePath, function(
                                    e,
                                    project_name
                                ) {
                                    window.isNowSaving = false;
            ...
            function getTranslatedObject(object) {
                ...
                if (object.sprite) {
                    if (
                        object.sprite.pictures &&
                        object.sprite.pictures.length > 0
                    ) {
                        for (
                            var i = 0, len = object.sprite.pictures.length;
                            i < len;
                            i++
                        ) {
                            object.sprite.pictures[
                                i
                            ].name = EntryStatic.getName(
                                object.sprite.pictures[i].name,
                                'picture'
                            );
                        }
                    }
                    if (
                        object.sprite.sounds &&
                        object.sprite.sounds.length > 0
                    ) {
                        for (
                            var i = 0, len = object.sprite.sounds.length;
                            i < len;
                            i++
                        ) {
                            object.sprite.sounds[i].name = EntryStatic.getName(
                                object.sprite.sounds[i].name,
                                'sound'
                            );
                        }
                    }
    ...
    .service('myProject', function($rootScope) {
        ...
        this.checkSavedProject = function() {
            return (
                !Entry.stateManager.isSaved() ||
                (this.isModeChange && !this.isSaved)
            );
        };
