...
function calcDurationForWav(audioHex) {
    ...
    var bit_per_sample = parseInt(
        getLittleToBigEndian(header.substr(24, 4)),
        16
    );
...
Entry.plugin = (function() {
    ...
    var getUploadPath = function(fileId, option) {
        ...
        if (
            !fs.existsSync(
                path.join(baseDir, fileId.substr(0, 2), fileId.substr(2, 2))
            )
        )
            fs.mkdirSync(
                path.join(baseDir, fileId.substr(0, 2), fileId.substr(2, 2)),
            ...
            function(filePath) {
                if (filePath) {
                    ...
                    fs.readFile(
                        ...
                        function(err, stream) {
                            fs.writeFile(filePath, stream, 'utf8', function(
                                err
                            ) {
                                if (err) alert('Unable to save file');
    ...
    that.init = function(cb) {
        ...
        var font = new FontFace(
            'nanumBarunRegular',
            'url(./node_modules/fonts/NanumBarunGothic.woff2)'
        );
        font.load();
        font.loaded.then(() => {
            ...
            if (!isNotFirst) {
                ...
                if (
                    localStorage.hasOwnProperty('tempProject') &&
                    isExistFolder
                ) {
                    isTempRecovery = confirm(
                        Lang.Workspace.restore_project_msg
                    );
                }
                ...
                if (!isTempRecovery) {
                    ...
                    var jsonObj = JSON.parse(
                        localStorage.getItem('tempProject')
                    );
    ...
    that.saveProject = function(filePath, data, cb, enc) {
        ...
        that.mkdir(path.join(_real_temp_path, 'temp'), function() {
            fs.writeFile(
                path.join(_real_temp_path, 'temp', 'project.json'),
                string_data,
                { encoding: enc || 'utf8', mode: '0777' },
                function(err) {
                    if (err) {
                        throw err;
                    }
    ...
    that.loadProject = function(filePath, cb, enc) {
        ...
        gunzip.on('end', function() {
            ...
            fs_writer.on('end', function() {
                fs.readFile(
                    path.resolve(_real_temp_path, 'temp', 'project.json'),
                    enc || 'utf8',
                    function(err, data) {
                        if (err) {
                            cb(err);
            ...
            bufferStream.pipe(tar.Parse()).pipe(fs_writer);
        });
                        ...
                        fs.writeFile(
                            ...
                            function(err) {
                                ...
                                var picture = {
                                    ...
                                    name: fileId,
                                    filename: fileId,
                                    fileurl: (dest.imagePath + '.png').replace(
                                        /\\/gi,
                                        '/'
                                    ),
        ...
        images.forEach(function(url, index) {
            ...
            that.mkdir(dest.uploadDir + '/image', function() {
                ...
                orgImage.onload = function() {
                    ...
                    var orgData = that.getResizeImageFromBase64(
                        orgImage,
                        orgCanvas,
                        960
                    );
                    ...
                    fs_writer.end(function() {
                        ...
                        var image = new Image();
                        Util.getMediaDataURI(imagePath)
                            .then((imageData) => {
                                image.src = imageData;
                            })
                            .catch((imageData) => {
                        ...
                        image.onload = function() {
                            ...
                            var thumb = that.getResizeImageFromBase64(
                                image,
                                canvas,
                                THUMB_SIZE
                            );
        ...
        images.forEach(function(image, index) {
            ...
            that.mkdir(dest.uploadDir + '/image', function() {
                ...
                orgImage.onload = function() {
                    ...
                    orgCanvas.width = orgImage.width;
                    orgCanvas.height = orgImage.height;
                    const orgData = that.getResizeImageFromBase64(
                        orgImage,
                        orgCanvas,
                        960
                    );
                    ...
                    fs_writer.end(function() {
                        ...
                        image.onload = function() {
                            ...
                            const thumb = that.getResizeImageFromBase64(
                                image,
                                canvas,
                                THUMB_SIZE
                            );
                    ...
                    fs.readFile(src, function(err, stream) {
                        ...
                        fs.writeFile(
                            ...
                            function(err) {
                                ...
                                Util.getMediaDataURI(soundPath)
                                    .then((soundData) => {
                                        audio.src = soundData;
                                    })
                                    .catch((err) => {
