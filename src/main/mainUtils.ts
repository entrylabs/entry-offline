...
export default class MainUtils {
     ...
     */
    static async captureBlockImage(image: any, filePath: string) {
        const FREE_SPACE = 25;
        ...
        try {
            ...
            ipcMain.handle('getImageString', () => {
                return image;
            })
            ipcMain.on('captureAndSave', async () => {
                ...
                await setTimeout(async () => {
                    ...
                    await FileUtils.writeFile(capturedImage.toPNG(), filePath);
                    captureWindow.close();
                }, WAITING_TIME);
            })
            captureWindow.addListener('closed', () => {
                ipcMain.removeAllListeners('captureAndSave');
                ipcMain.removeHandler('getImageString');
            })
        } catch (error) {
            console.error(error);
        }
